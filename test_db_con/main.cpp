...
public:
    ...
    virtual void open() {
        std::cout << "Подключаемся в базе данных\n";
    }

    virtual void close() {
        std::cout << "Отключаемся от базы данных\n";
    }

    virtual std::string execQuery(const std::string& query) {
        std::cout << "Выполняем запрос в бд: " << query << "\n";
        return "Возвращаем результат запроса";
    }
};

// Класс ClassThatUsesDB
